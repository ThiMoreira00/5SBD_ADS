...
CREATE DEFINER=`root`@`localhost` PROCEDURE `processa_entrega` ()   BEGIN
    ...
    DECLARE cursor_pedidos CURSOR FOR
        ...
        FROM sbd_itens_pedido ip
        JOIN sbd_produtos p ON ip.produto_id = p.produto_id
    ...
    read_loop: LOOP
        ...
        SELECT quantidade INTO v_estoque_atual FROM sbd_estoque WHERE produto_id = v_id_produto;
        
        -- Se o estoque for suficiente
        IF v_estoque_atual >= v_quantidade THEN
            -- Atualiza a tabela entrega com o total do pedido
            INSERT INTO sbd_entregas (pedido_id, produto_id, quantidade, total_item)
        ...
        ELSE
            ...
            INSERT INTO sbd_compras (produto_id, quantidade_necessaria)
